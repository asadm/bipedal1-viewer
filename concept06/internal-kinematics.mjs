import {leg as originalLeg} from './kinematics.mjs';
export function leg(g,q){
 if(g.lever_phase_rad===undefined)return originalLeg(g,q);
 ...
 if(square<=0)throw Error('Internal linkage cannot close');
 const k=Math.sqrt(square),D=[C[0]+h*dx/d+k*dz/d,C[1]+h*dz/d-k*dx/d];
 const phi=(Math.atan2(-(D[1]-C[1]),D[0]-C[0])-g.lever_phase_rad+Math.PI*4)%(Math.PI*2);
 return{C,D,E:[C[0]+l*Math.cos(phi),C[1]-l*Math.sin(phi)],phi,beta:Math.atan2(-(D[1]-B[1]),D[0]-B[0]),knee:phi-q};
}
...
export function pose(p,heights){
 ...
 for(let i=0;i<2;i++){
  ...
  const u=p.spring.upper_seat_from_knee_m,v=p.spring.lower_seat_from_knee_m,sy=sign*(.052+p.spring.seat_y_from_leg_m);
  seats.push([[x+s.C[0]/1000-u*Math.cos(q),sy,z+s.C[1]/1000+u*Math.sin(q)],[x+s.C[0]/1000+v*Math.cos(s.phi),sy,z+s.C[1]/1000-v*Math.sin(s.phi)]]);
 }
