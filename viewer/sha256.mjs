// Asset integrity on both secure origins and plain HTTP LAN/Tailscale URLs.
// Use native SHA-256 where available; the fallback implements the same digest.
...
export function sha256Fallback(input) {
  const source=bytesOf(input), length=source.byteLength;
...
export async function sha256(input) {
  if (!globalThis.crypto?.subtle) return sha256Fallback(input);
  const digest=await globalThis.crypto.subtle.digest('SHA-256',bytesOf(input));
