// Generated by scripts/cad/build_detail_viewer.py; frozen replay source stays unchanged.
...
import {nativeReplay} from './v2-replay.mjs?v=detail-recovery-1';
...
try {
  ...
  const inFocus = p => detailStudy ? p.source_component === 'assembly_detail' && !p.name.startsWith('body_') : outputStudy
  ...
  const recordedParts = parts;
  let detailStudy = null;
  if (study === 'output' && query.get('revision') !== 'recorded') {
    const [manifestResponse, partsResponse] = await Promise.all([
      fetch('../clanky-v2/detail/manifest.json'), fetch('../clanky-v2/detail/parts.json')
    ]);
    if (!manifestResponse.ok || !partsResponse.ok) throw Error('Assembly detail export is not ready');
    const detailManifestBytes = await manifestResponse.arrayBuffer();
    detailStudy = JSON.parse(new TextDecoder().decode(detailManifestBytes));
    const raw = await partsResponse.arrayBuffer();
    if (await sha256(raw) !== detailStudy.parts_sha256) throw Error('Assembly detail CAD checksum mismatch');
    await Promise.all(Object.entries(detailStudy.baseline_source_sha256).map(async ([name, expected]) => {
      const path = name.startsWith('viewer/') ? '../' + name : '../clanky-v2/' + name;
      const r = await fetch(path);
      if (!r.ok || await sha256(await r.arrayBuffer()) !== expected) throw Error('Assembly detail needs rebuilding: ' + name);
    }));
    const removed = new Set(detailStudy.removed_parts);
    parts = [...parts.filter(p => !removed.has(p.name)), ...JSON.parse(new TextDecoder().decode(raw))];
    if (parts.length !== detailStudy.total_parts || new Set(parts.map(p=>p.name)).size !== parts.length) throw Error('Assembly detail parts do not match the manifest');
    document.title = 'Clanky V2 / Assembly details';
    $('caption').textContent = 'Latest assembly details · manual pose inspection';
    document.querySelector('h1').textContent = 'Clanky, inside and out.';
    document.querySelector('.intro').textContent = 'Enclosed bending legs, one-piece 120 mm wheels and four motors. The body includes D435i supports, Pi posts and battery strap bridges. The default jump now uses this detailed CAD and its updated mass.';
    $('fit-summary').textContent = 'Latest CAD: programmed balance and jump checked at two timesteps. Cooler fit, print calibration, wiring flex and hardware loads remain open.';
    $('fit-evidence').textContent = 'The new parts are separate valid CAD solids, bound to this revision by checksum. Earlier joint and motion reports do not qualify these additions.';
    $('fit-evidence').nextSibling.textContent = ' The remaining reported overlaps concern the cooler reservation and supplier set-screw threads. They remain open. Print fits, load capacity and assembly qualification are unfinished.';
    try {
      const [meshResponse, exactResponse] = await Promise.all([
        fetch('../clanky-v2/detail/mesh-diagnostic.json'), fetch('../clanky-v2/detail/exact-diagnostic.json')
      ]);
      const meshBytes = await meshResponse.arrayBuffer(), exact = await exactResponse.json();
      const mesh = JSON.parse(new TextDecoder().decode(meshBytes));
      if (exact.source_sha256['detail/manifest.json'] !== await sha256(detailManifestBytes) ||
          exact.source_sha256['detail/parts.json'] !== detailStudy.parts_sha256 ||
          exact.source_sha256['detail/mesh-diagnostic.json'] !== await sha256(meshBytes) ||
          mesh.source_sha256['detail/parts.json'] !== detailStudy.parts_sha256) throw Error('stale detail audit');
      const clear = exact.cases.filter(c=>c.passed).length;
      $('fit-evidence').textContent = `${mesh.poses} sampled leg poses; ${mesh.mesh_checks.filter(c=>c.passed).length}/${mesh.mesh_checks.length} meshes pass topology; ${clear}/${exact.cases.length} exact contact checks have no positive overlap. ${exact.untested_pairs.length} reported pairs are untested.`;
      $('fit-summary').textContent = `Latest CAD: ${clear}/${exact.cases.length} exact contact checks clear. Nominal gear-cycle and paired jump checks are linked below; cooler and supplier thread overlaps remain open.`;
    } catch { /* Keep the new CAD explicitly unqualified without matching evidence. */ }
    document.querySelector('h2').textContent = 'Fewer loose brackets';
    document.querySelector('.anatomy').innerHTML = '<li><span>01</span><div><strong>Both hips detailed</strong><p>Stock output gears, integral thigh flanges, metal spacers and supported spindles on both sides.</p></div></li><li><span>02</span><div><strong>Room for the camera</strong><p>The D435i bolts to two rear supports in the tray. Its native optical face stays exposed, with room beside the camera for USB.</p></div></li><li><span>03</span><div><strong>Electronics stay attached</strong><p>Four captive-nut posts hold the Pi to the removable cover, components facing down. Battery strap bridges sit in the tray. Cooler and cable fit are still being checked.</p></div></li>';
    document.querySelector('details .downloads').innerHTML = '<a href="../clanky-v2/detail/parts-download.zip">Detail parts STEP + notes (ZIP) ↓</a><a href="../clanky-v2/detail/manifest.json">Dimensions and remaining work ↗</a><a href="../clanky-v2/detail/exact-diagnostic.json">Exact clearance diagnostic ↗</a><a href="../clanky-v2/detail-gear/exact-cycle.json">Gear tooth-cycle checks ↗</a><a href="../clanky-v2/detail-native/baseline-trials.json">Latest balance and jump checks ↗</a><a href="../clanky-v2/detail-native/onewheel/hold-summary.json">One-wheel attempts and limitations ↗</a><a href="./print-fit.html">Print fits and calibration samples ↗</a>';
    document.querySelector('option[value="manual"]').textContent = 'Manual · latest assembly details';
    $('drive-focus').textContent = 'Inspect new mechanical details';
  }
  ...
  let robot = cadRobot(parts.map(p=>({...p,color:p.name.startsWith('left_output_gear_')?[.66,.71,.73]:p.source_component==='hip_drive'&&p.kind==='stock_gear'?(p.sku.startsWith('2302')?[.66,.71,.73]:[.22,.26,.28]):colors[p.material]})),parameters);
  const detailRobot = robot;
  const recordedRobot = detailStudy ? cadRobot(recordedParts.map(p=>({...p,color:colors[p.material]})), parameters) : robot;
  ...
  const scene = new THREE.Scene(); scene.background = new THREE.Color('#edf1ef'); scene.add(robot.root); if (recordedRobot !== robot) { scene.add(recordedRobot.root); recordedRobot.root.visible = false; }
  ...
  if (driveStudy) { $('fold-size').textContent='Pending complete fit audit'; $('prints').textContent=parts.filter(p=>p.printed).length; }
  ...
  function pose() {
    const recorded = replay?.active && replay.cadRevision === 'recorded';
    robot = recorded ? recordedRobot : detailRobot;
    if (detailRobot !== recordedRobot) {
      detailRobot.root.visible = !recorded;
      recordedRobot.root.visible = !!recorded;
    }
    ...
    if (detailStudy) {
      $('view-note').textContent = replay?.active
        ? (recorded ? 'Preceding CAD revision: this older recording retains its original model and physics.' : 'Latest detailed CAD: recorded MuJoCo motion with updated mass and rotating gears. Physics view uses this same model.')
        : ({enclosed:'Latest assembly details: both hips, integral electronics mounts and recessed closure screws. Clearance validation is in progress.',cutaway:'Transparent shells reveal motor mounts, camera supports, Pi posts, rotor adapters and battery strap passages. Amber components are unfinished or reserved space.',packaging:'Roomy camera and electronics bays with mounting hardware. Cooler fit, connector plugs and cable flex remain in progress.',physics:'Latest CAD collision shapes, placed at the manual pose. Select a recorded motion to inspect its physics.',drive:'New mechanical details. Both leg sliders remain available in Manual mode.'}[display]);
      document.querySelector('.object-label span:last-child').textContent = recorded ? 'Preceding CAD revision' : 'Latest detailed CAD';
      $('prints').textContent = (recorded ? recordedParts : parts).filter(p=>p.printed).length;
      document.querySelector('.sidebar-footer').textContent = recorded ? 'Recorded simulation · preceding CAD revision' : replay?.active ? 'Latest detailed CAD · recorded simulation' : 'Latest assembly details · manual inspection';
      document.querySelector('.fine-print').textContent = 'Programmed jump: 264–265 mm wheel clearance and a settled landing at both tested timesteps. Other motions have their own results above. This is nominal simulation, not physical validation. The folded width exceeds the former 180 mm target.';
    }
    ...
    else if (replay) $('caption').textContent = detailStudy ? 'Latest assembly details · manual pose inspection' : 'Latest V2 design · manual pose inspection';
    ...
    if (driveStudy) robot.root.traverse(o=>{
      ...
      if (outputStudy && p.name.endsWith('driven_thigh_frame') && display==='drive') {o.material.opacity=.55;o.material.depthWrite=false;}
      ...
      if (p.name.startsWith('body_lower')||p.name.startsWith('body_upper')) {
        ...
        o.material.transparent=!opaque;o.material.opacity=opaque?1:display==='enclosed'?.22:.14;
  ...
  if (outputStudy) {
    ...
    try {
      replay = await nativeReplay(scene, refit=>{pose();if(refit)fit();}, camera, controls, {detailed:!!detailStudy});
      ...
      if (!detailStudy) $('fit-evidence').nextSibling.textContent = ' Gear tooth phase, input and compound torque connections, body closure, tolerances, fastening and strength remain unfinished. Manual poses are prescribed; the motion dropdown contains separately recorded V2 experiments.';
      document.querySelector('.fine-print').textContent='The wider body gives the D435i and its USB cable room. This layout exceeds the former 180 mm folded-width target. The latest detailed CAD demonstrates 264–265 mm wheel clearance in the two nominal programmed trials.';
